...
class AndroidNDKConan(ConanFile):
    ...
    def package_info(self):
        ...
        self.buildenv_info.define_path("ANDROID_NDK_ROOT", os.path.join(self.package_folder, "bin"))   # {package_folder}/bin/toolchains/... 

        self.buildenv_info.define_path("ANDROID_NDK_PATH",  os.path.join(self.package_folder, "bin"))

        # According to the rules of NDK-Path https://github.com/android/ndk-samples/wiki/Configure-NDK-Path
        # You can not specify both ndkVersion and ndkPath if you want an explicit ndkPath
        self.cpp_info.set_property("ANDROID_NDK", os.path.join(self.package_folder, "bin"))  # This is how dfx-embedded-opencv initializes
        ...
        # Implication: Need to have build.gradle() set android.ndkPath and not set android.ndkVersion
